...
const EXTENDED = [PUJA_SERVICES[N - 1], ...PUJA_SERVICES, PUJA_SERVICES[0]];

/**
 * Mobile-only (< md) peek carousel for the Puja cards — previous card and
 * next card visible at the edges, active card centered, swipeable. Tablet
 * and desktop keep the plain grid (see PujaSection). No autoplay: unlike
 * the hero banners this is a browsable list of services, not a rotating
 * promo, so it only moves on user input.
 */
export function PujaCarousel() {
  ...
  const slideHeightPx = slideWidthPx / CARD_ASPECT;
  ...
  return (
    ...
    >
      ...
      <p className="sr-only" aria-live="polite">
        {`Showing ${PUJA_SERVICES[activeRealIndex].title}, ${activeRealIndex + 1} of ${N}`}
      </p>
    </div>
  );
}
