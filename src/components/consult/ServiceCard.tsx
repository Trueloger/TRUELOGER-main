...
// name, subtitle, duration range, a default 30-min price, and two
// actions — "Add to Cart" (opens the DurationSheet, the DIRECT
// add-to-cart flow) and "View More" (a real Link to the subpage).
    ...
    <div className="group relative flex flex-col rounded-[1.4rem] border border-nav-lavender-line bg-gradient-to-b from-white to-nav-lavender-mist p-5 text-center shadow-[0_10px_26px_-16px_rgba(70,40,120,0.35)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_34px_-14px_rgba(70,40,120,0.4)] sm:text-left">
      ...
      <div className="flex flex-col items-center sm:flex-row sm:items-start sm:gap-4">
        ...
        <div className="mt-3 min-w-0 sm:mt-0">
          <h3 className="font-serif text-lg leading-snug text-nav-violet">{service.name}</h3>
          <p className="mt-1 text-sm leading-snug text-nav-plum/75">{service.subtitle}</p>
        </div>
      </div>
      ...
      <div className="mt-2 flex items-baseline justify-center gap-1.5 sm:justify-start">
        ...
        <span className="text-xs text-nav-plum/60">30 min · full range 15–60 min</span>
      </div>
