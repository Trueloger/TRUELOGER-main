...
import type { NakshatraApiResponse } from "./types";
...
/** Nakshatra (lunar mansion) tool: BirthDetailsForm -> POST
 * /api/nakshatra -> loading/error/result. Mirrors NumerologyForm.tsx's
 * flow, adapted to a real external-API call and the shared
 * birth-details form. Time of birth is required (matters near a
 * nakshatra boundary); checking "I don't know my exact birth time"
 * defaults to noon and the result discloses that. */
export function NakshatraForm() {
  ...
  const [result, setResult] = useState<NakshatraApiResponse | null>(null);
  ...
  async function generate() {
    ...
    try {
      ...
      const data = (await res.json()) as NakshatraApiResponse | { error: string };
  ...
  if (status === "loading") {
    return <LoadingState message="Reading the stars for your Nakshatra…" />;
  }
    ...
    return (
      <div className="space-y-8">
        <h1 ref={resultHeadingRef} tabIndex={-1} className="sr-only">
          Your Nakshatra Reading
        </h1>

        <ResultHeader
          title="Your Nakshatra"
          subtitle={`${calculated.nakshatraName}, Pada ${calculated.pada}, for ${values.name.trim()}`}
        />
        ...
        <div className="rounded-2xl border border-nav-lavender-line bg-nav-pearl p-6 text-center">
          ...
          <p className="mt-1 font-serif text-3xl text-nav-plum">{calculated.nakshatraName}</p>
          <div className="mt-5 grid grid-cols-2 gap-4 text-left sm:grid-cols-4">
            <div>
              <p className="text-[0.7rem] uppercase tracking-wide text-nav-plum/50">Pada</p>
              <p className="text-sm text-nav-plum">{calculated.pada}</p>
            </div>
            <div>
              <p className="text-[0.7rem] uppercase tracking-wide text-nav-plum/50">
                Ruling Lord
              </p>
              <p className="text-sm text-nav-plum">{calculated.vimsottariLord}</p>
            </div>
            <div>
              <p className="text-[0.7rem] uppercase tracking-wide text-nav-plum/50">Deity</p>
              <p className="text-sm text-nav-plum">{calculated.deity ?? "—"}</p>
            </div>
            <div>
              <p className="text-[0.7rem] uppercase tracking-wide text-nav-plum/50">Symbol</p>
              <p className="text-sm text-nav-plum">{calculated.symbol ?? "—"}</p>
            </div>
          </div>
          <p className="mt-4 text-sm text-nav-plum/60">
            Moon in {calculated.moonSign}, {calculated.degreeInSign.toFixed(2)}° into the sign.
          </p>
        </div>
        ...
        {calculated.timeUnknown && (
          <p className="text-center text-sm text-nav-plum/60">
            ...
            nakshatra boundary, an unknown birth time can shift the calculated result — for full
        ...
        {result.reportError && (
          <p className="text-center text-sm text-nav-plum/60">
            Your calculated Nakshatra above is accurate. We couldn&apos;t generate the extended
  ...
  return (
    <div id="nakshatra-form" className="mx-auto max-w-md">
      <BirthDetailsForm
        values={values}
        ...
        submitLabel="Generate My Nakshatra"
        submitting={false}
      />
    </div>
  );
