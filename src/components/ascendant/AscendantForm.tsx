...
import type { AscendantApiResponse } from "./types";
...
/** Ascendant (Lagna/rising sign) tool: BirthDetailsForm -> POST
 * /api/ascendant -> loading/error/result. Mirrors NumerologyForm.tsx's
 * flow, adapted to a real external-API call and the shared
 * birth-details form. Time of birth is required — the Ascendant
 * changes roughly every two hours, so it absolutely needs an exact
 * time; checking "I don't know my exact birth time" defaults to noon
 * and the result discloses that this can shift the result
 * significantly. */
export function AscendantForm() {
  ...
  const [result, setResult] = useState<AscendantApiResponse | null>(null);
  ...
  async function generate() {
    ...
    try {
      ...
      const data = (await res.json()) as AscendantApiResponse | { error: string };
  ...
  if (status === "loading") {
    return <LoadingState message="Reading the stars for your Ascendant…" />;
  }
    ...
    return (
      <div className="space-y-8">
        <h1 ref={resultHeadingRef} tabIndex={-1} className="sr-only">
          Your Ascendant Reading
        </h1>

        <ResultHeader
          title="Your Ascendant"
          subtitle={`${calculated.signName} Rising, for ${values.name.trim()}`}
        />
        ...
        {calculated.timeUnknown && (
          <p className="text-center text-sm text-nav-plum/60">
            Your birth time wasn&apos;t provided, so we used noon (12:00) as a default. The
            Ascendant changes roughly every two hours, so an unknown birth time can shift this
            result significantly — treat it as approximate until you can provide your exact
            birth time.
          </p>
        )}
        ...
        {result.reportError && (
          <p className="text-center text-sm text-nav-plum/60">
            Your calculated Ascendant above is accurate. We couldn&apos;t generate the extended
  ...
  return (
    <div id="ascendant-form" className="mx-auto max-w-md">
      <BirthDetailsForm
        values={values}
        ...
        submitLabel="Generate My Ascendant"
        submitting={false}
      />
    </div>
  );
